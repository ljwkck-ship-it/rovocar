...
- 단어장 기본 정렬은 최신 등록순이며, 가장 최근에 만든 단어장이 `CAR 01`로 표시
- 영어→한글, 한글→영어 양방향 입력 퀴즈
- 한글 뜻이 여러 개면 쉼표·가운뎃점·슬래시·세미콜론·`또는`으로 나눈 뜻 중 하나만 맞아도 정답 처리
- 영어 발음은 기기의 시스템 음성을 사용하며, macOS에서는 `Samantha`, `Aaron`, `Alex` 순으로 자연스러운 영어 음성을 우선 선택
- 정답·오답·풀이 횟수와 학습 상태 저장
- 영어 철자 기준으로 사용자가 몇 번째 등록한 고유 단어인지 표시
- 같은 영어 단어가 여러 번 등록되면 퀴즈 화면에만 `중복 2번째`처럼 짧게 표시
- 로그인 전 기기 저장과 로그인 후 Supabase 동기화
- 네트워크가 없어도 사용할 수 있는 오프라인 캐시와 서비스 워커

중복 단어는 CSV 등록이나 직접 추가 단계에서 막지 않습니다. 뜻 맞히기와 스펠링 맞히기 화면에서만 기존 단어 번호 옆에 `· 중복 N번째`를 일반 텍스트로 표시합니다. 최초 등록 단어에는 중복 문구가 붙지 않습니다.

### 모바일 화면 보정

iPhone PWA 캡처와 393px 모바일 뷰포트를 기준으로 다음 화면을 다시 점검했습니다.

- 단어장 카드에서 긴 이름과 정답률·주행 기록이 겹치던 문제 수정
- 단어 번호, 영어 스펠링, 한글 뜻, 수정 버튼이 겹치지 않도록 모바일 그리드 분리
- 긴 영어 단어와 여러 줄 한글 뜻에서도 행 높이가 자연스럽게 늘어나도록 처리
- 수정된 CSS가 설치형 PWA에도 내려가도록 서비스 워커 캐시 버전 갱신

실제 운영 데이터뿐 아니라 긴 테스트용 단어장 이름, `extraordinarilylongspelling` 같은 긴 철자와 여러 줄 뜻을 넣어 첫 화면과 단어 목록을 각각 렌더링했습니다.
...
카카오 로그인은 필수가 아닙니다. 로그인하지 않아도 단어장 생성, CSV 등록, 뜻 맞히기, 스펠링 맞히기와 학습 기록 저장을 모두 사용할 수 있습니다. 데이터는 브라우저 세션이 끝날 때 사라지는 `sessionStorage`가 아니라 해당 기기와 브라우저의 `localStorage`에 저장되므로 브라우저를 닫았다 다시 열어도 유지됩니다. 단, 브라우저 데이터 삭제, 시크릿 모드 종료, PWA 삭제, 다른 브라우저나 다른 기기로 이동하면 복원되지 않습니다.

### 동기화 안정화

로그아웃 후 일부 단어장이 사라지고 `duplicate key value` 오류가 표시되던 문제를 운영 환경에서 재현해 수정했습니다. 원인은 단어장별로 다시 시작하던 등록 번호가 사용자 단위 고유 제약조건과 충돌한 것이었습니다.

- 등록 번호를 로그인 사용자 전체에서 하나의 연속 번호로 관리
- 같은 영어 철자는 여러 단어장에 있어도 최초 등록 번호를 공유
- 서로 다른 단어에 같은 번호가 배정된 기존 로컬 데이터를 동기화 전에 자동 보정
...
- 카테고리: 교육
- iPhone PWA 재설치 직후에도 인증 이벤트가 중복으로 화면을 초기화하지 않도록 같은 사용자 세션은 한 번만 처리
- 초기 동기화 중 단어장을 생성·삭제하면 최신 로컬 상태를 우선 저장하고, 서버 응답이 차고 목록을 되돌리지 않도록 처리
- 저장 중인 동기화가 있으면 완료까지 기다린 뒤 가장 최신 단어장 상태를 순서대로 저장
- 서비스워커 캐시를 `rovocar-v17`로 갱신해 설치형 PWA와 Safari 웹에서 같은 수정본을 받도록 처리
...
RoVoCar는 설치하지 않고 Safari나 Chrome에서 일반 웹앱으로 바로 사용할 수 있습니다. 자주 쓴다면 앱스토어를 거치지 않고 홈 화면이나 응용 프로그램으로 설치할 수도 있습니다. 웹 사용과 PWA 설치는 기능상 필수 조건이 아니며, 로그인 여부와도 별개입니다.

설치할 때는 반드시 운영 주소 <https://ljwkck-ship-it.github.io/rovocar/>를 사용합니다. `localhost` 주소는 개발용이므로 가족 기기 설치에 사용하지 않습니다.
...
- PWA로 설치하지 않아도 운영 주소에서 모든 학습 기능을 사용할 수 있습니다.
- 카카오 로그인 없이도 웹과 설치형 PWA 모두 사용할 수 있습니다.
- 비로그인 데이터는 해당 기기와 브라우저 또는 설치 앱의 로컬 저장소에만 남습니다.
...
기존 단어장에 추가할 때는 단어장 메뉴의 CSV 추가 기능을 사용합니다. 같은 영어 철자가 이미 등록된 경우에도 단어장에는 포함할 수 있지만, 사용자의 고유 단어 등록 순번은 최초 등록 기준으로 유지됩니다. 등록 단계에서는 중복 경고를 띄우지 않습니다. 두 퀴즈 모드에서 해당 단어가 나오면 `MY WORD #0160 · 중복 2번째`처럼 짧게 확인할 수 있습니다.
...
- 첫 줄: `English,Korean`
- 열 순서 고정: 첫 번째 열은 영어 스펠링, 두 번째 열은 한글 뜻
- 한 행에 영어 단어 하나와 대응하는 한글 뜻 입력
- 한글 뜻이 여러 개면 `수반하다, 포함하다`처럼 한 셀에 함께 입력
- 값에 쉼표가 있으면 큰따옴표로 감싸기
- UTF-8 CSV 권장

`Korean,English`처럼 열을 반대로 넣으면 등록은 될 수 있지만 뜻 맞히기, 스펠링 맞히기, 영어 발음과 중복 판정이 잘못 동작합니다. 한글 뜻에 쉼표가 들어가면 CSV 셀 전체를 큰따옴표로 감싸야 합니다.
...
비로그인 단어장과 학습 상태는 브라우저 탭이나 창을 닫아도 유지됩니다. 브라우저 데이터 삭제, 시크릿 모드 종료, PWA 삭제, 다른 브라우저나 새 기기에서는 로컬 데이터와 로그인 세션이 유지되지 않을 수 있습니다.
...
- 관리자 RLS 접근 성공
- 비관리자 접근 차단 구조 확인
- 393px iPhone 크기에서 단어장 카드와 단어 목록 겹침 없음
- 서로 다른 단어장의 동일 철자에 같은 등록 번호 적용
- 중복 단어의 등록 순서를 두 퀴즈 모드에서만 표시
- 로그아웃 직전 강제 동기화 후 재로그인 데이터 유지
- 목표 단어장 3개가 재로그인 후 모두 복원되는 것 확인
...
- 실제 iPhone에서 새 캐시 적용 후 다양한 글자 길이 추가 점검
- 카카오 User Unlinked 웹훅 구성
- 개인정보 처리방침 페이지 추가
- 관리자 화면의 기간별 접속 추이와 CSV 내보내기(개인정보 최소화 유지)
- iPhone 설치형 PWA에서 단어장 삭제 뒤 차고 목록이 즉시 감소하고 유지되는 흐름 확인
- Safari 웹과 설치형 PWA가 같은 동기화 로직을 사용하도록 확인
